...
app.post("/api/convert", (req, res) => {
  ...
  form.parse(req, (err, fields, files) => {
    ...
    if (format === 'docx') {
    // Try different approaches for DOCX conversion
    // Approach 1: Use writer_pdf_Export filter
    cmd = `soffice --headless --convert-to docx:"MS Word 2007 XML" "${filePath}" --outdir ./temp`;
  } else if (format === 'pdf') {
    // PDF conversion with specific filter
...
setTimeout(() => {
  try {
    ...
    // Find any file with the target extension
    ...
    // If still not found, check for any .docx file (for DOCX conversion)
    if (format === 'docx') {
