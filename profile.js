...
function updateDeletePhotoButton() {
  ...
  console.log("updateDeletePhotoButton called, currentUser.photo:", currentUser ? currentUser.photo : 'null');

  // Check if user has a photo that is not the default
  ...
  if (hasPhoto) {
    ...
    deletePhotoBtn.onclick = async function(e) {
      ...
      try {
        // Update database - set photo to empty string
    ...
    console.log("Delete photo button added");
  } else {
    console.log("No photo - delete button hidden");
  }
}
...
if (saveAccountBtn) {
  saveAccountBtn.addEventListener("click", async function() {
    ...
    try {
      ...
      toastInfo("Verification code sent to your email. Remember to check your SPAM/JUNK folder.");
