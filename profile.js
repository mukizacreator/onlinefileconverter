...
async function loadUserData() {
  try {
    ...
    // Update navigation - ALWAYS sync with database
...
function updateDeletePhotoButton() {
  ...
  if (currentUser && currentUser.photo && currentUser.photo !== DEFAULT_ICON) {
...
if (uploadPhotoBtn && profilePhotoInput) {
  ...
  profilePhotoInput.addEventListener("change", function(e) {
    ...
    reader.onload = async function(event) {
      ...
      try {
        // Use FormData instead of JSON for large data
...
if (saveAccountBtn) {
  saveAccountBtn.addEventListener("click", async function() {
    ...
    try {
      ...
      // Reload user data to update navigation
      await loadUserData();
