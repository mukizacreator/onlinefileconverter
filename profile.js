...
/* ============================================
   LOAD USER DATA INTO PROFILE - FIXED
   ============================================ */
function loadUserData() {
  ...
  if (profileUsername) {
    ...
    console.log("Profile username set to:", currentUser.username);
  }
  if (profileEmailDisplay) {
    profileEmailDisplay.textContent = currentUser.email || 'user@email.com';
    console.log("Profile email set to:", currentUser.email);
  }
  
  // Update Profile View (default)
  if (profileViewUsername) {
    profileViewUsername.textContent = currentUser.username || 'User';
    console.log("Profile view username set to:", currentUser.username);
  }
  if (profileViewEmail) {
    profileViewEmail.textContent = currentUser.email || 'user@email.com';
    console.log("Profile view email set to:", currentUser.email);
  }
  
  // Update Account Panel
  if (accountUsernameDisplay) {
    accountUsernameDisplay.textContent = currentUser.username || 'User';
    console.log("Account username display set to:", currentUser.username);
  }
  if (accountEmailDisplay) {
    accountEmailDisplay.textContent = currentUser.email || 'user@email.com';
    console.log("Account email display set to:", currentUser.email);
  }
  
  // Update form fields
  if (profileEmail) {
    profileEmail.value = currentUser.email || '';
    console.log("Profile email field set to:", currentUser.email);
  }
  if (profileUsernameInput) {
    profileUsernameInput.value = currentUser.username || '';
    console.log("Profile username field set to:", currentUser.username);
  }
  
  // Update profile photo - ALWAYS sync
  if (profileImage) {
    if (currentUser.photo) {
      profileImage.src = currentUser.photo;
      console.log("Profile image set to user photo");
    } else {
      profileImage.src = "https://cdn-icons-png.flaticon.com/512/149/149071.png";
      console.log("Profile image set to default");
    }
  }
  ...
  if (navUsername) {
    navUsername.textContent = currentUser.username || 'Profile';
    console.log("Nav username set to:", currentUser.username);
  }
  ...
  if (navProfilePhoto) {
    if (currentUser.photo) {
      ...
      console.log("Nav photo set to user photo");
    } else {
      navProfilePhoto.src = "favicon.png";
      console.log("Nav photo set to default favicon");
    }
  }
...
// CALL loadUserData IMMEDIATELY and also after any changes
loadUserData();
...
/* ============================================
   ACCOUNT INFORMATION UPDATE - FIXED
   ============================================ */
if (saveAccountBtn) {
  saveAccountBtn.addEventListener("click", async function() {
    ...
    try {
      ...
      // Reload user data to update all displays
      loadUserData();
...
if (logoutBtn) {
  logoutBtn.addEventListener("click", async function() {
    ...
    if (confirm) {
      console.log("Logout confirmed - removing user");
      localStorage.removeItem("loggedInUser");
...
if (deleteAccountBtn) {
  deleteAccountBtn.addEventListener("click", async function() {
    ...
    try {
      ...
      console.log("Deleting account:", currentUser.email);
...
/* ============================================
   RUN ON PAGE LOAD - ENSURE DATA IS LOADED
   ============================================ */
console.log("Profile.js loaded successfully");
console.log("Current user at end:", currentUser);

// Ensure data is loaded after everything
setTimeout(function() {
  console.log("Re-loading user data after page load");
  loadUserData();
}, 500);