...
if (fileInput && result && fileName && convertBtn) {
  ...
  convertBtn.addEventListener("click", async () => {
    ...
    /* ============================================
     ...
     ============================================*/
    // Guests are limited to 3MB files, logged-in users have no restriction
    const loggedInUser = localStorage.getItem("loggedInUser");
    const maxFreeSize = 3 * 1024 * 1024;

    if (file.size > maxFreeSize && !loggedInUser) {
      toastWarning("File is larger than 3 MB. Please sign in to convert larger files.");
      setTimeout(() => { 
      result.innerHTML = `
      <div style="background: rgba(255, 255, 255, 0.08); padding:20px; border-radius:15px;">
      <p style="color:#ffcc80; line-height:1.8;">Your file is larger than 3 MB.<br><br>Please sign in or create an account to convert larger files.</p>
